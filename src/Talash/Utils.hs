...
searchWithMatcher :: SearchFunctions a -- ^ The configuration to use to carry out the search.
  ...
  -> U.Vector  Int -- ^ The subset of indices of `v` to search against. If input changes from `talas` to `talash` we only search among candidates that match `talas`.
...
-- | Search functions suitable for fuzzy matching. The candidate `c` will match string `s` if `c` contains all the characters in `s` in order. In general there
